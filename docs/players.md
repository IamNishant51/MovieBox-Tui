...
The `supports_headers` gate in `app/playback.rs` validates whether a player can satisfy required stream headers. For VLC, signed CloudFront `Cookie` streams (such as MovieBox DASH manifests) are supported via an automatic detached loopback sidecar proxy (`--proxy-for-vlc`). The proxy injects the signed CloudFront cookies and auth headers server-side and streams chunks directly to VLC, running as an independent process group on Unix and a detached process on Windows so playback survives terminal closure. If an unproxied player cannot satisfy a stream's headers (such as Android Intent attempting to play signed Cookie streams), playback will not silently fall back to an alternative player. Instead, an explicit warning notification informs the user of the lack of header support, prompting them to switch to a supported player or install one for their operating system (`mpv`, `IINA`, or `VLC`).
## Subtitles
